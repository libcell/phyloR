#' Coalescent Tree Construction from Multiple Genes
#'
#' It constructs a coalescent phylogenetic tree by fetching gene sequences
#' from a given list of gene IDs, aligning and trimming them, and then building the tree
#' using a slected tree-building method. It retrieves orthologous gene sequences
#' for the slected species, processes them, and outputs a coalescent tree based on the selected
#' alignment and tree-building methods.
#'
#' @param gene_ids A character vector of gene IDs to be used for building the coalescent tree.
...
#'   2. Aligns and trims the sequences based on the slected alignment method.
...
#' @export
#'
coalescent_tree_fetch <- function(gene_ids,
                                  id.type = "ko_id",
                                  species.list = NULL,
  ...
  for (i in 1:length(gene_ids)) {
    ...
    file.name <- paste(gene_ids[i], "cal", sep = "_")
